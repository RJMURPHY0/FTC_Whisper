...
class Transcriber:
    ...
    def __init__(
        ...
        if model_size not in self.VALID_MODELS:
            print(f"[Transcriber] Invalid model '{model_size}', falling back to 'base.en'.")
            model_size = "base.en"
        ...
        print(f"[Transcriber] model={model_size!r}  device={device}  "
    ...
    def _post_process(self, text: str) -> str:
        text = text.strip()
        for artifact in ("[BLANK_AUDIO]", "[MUSIC]", "[SOUND]",
                         "(music)", "(silence)", "..."):
